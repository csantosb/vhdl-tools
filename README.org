...
allow auto folding when jumping around.
...
When the =vOrg= file exists, and assuming its names are identical, and only its
extension changes, it is possible to jump from the source vhdl file to the org
file with help of

  + =C-C M-^=    jump from vhdl to org

Once =vOrg= is active the following keys are made available

  + =C-c M-,=    jump from org to vhdl
  + =C-c C-n=    get to next header
  + =C-c C-p=    get to previous header
