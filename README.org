...
:END:
 ...
 - [[#use][Use]]
   ...
   - [[#misc][Misc]]
   - [[#vorg][vOrg]]
     - [[#vorg-link-tangling][vOrg link tangling]]
       - [[#vorg-tangling-extra][vOrg tangling extra]]
...
Setting =vhdl-tools-manage-folding= (off by default) to true will allow auto
folding upon jumping, which should work regardless of
the =vhdl-tools-tangle-comments-link= flag explained in next section.
...
back to the original org file, as a reference for jumping back and forth between =vorg=
and =vhdl=.

In next code =vhdl-tools-vorg-mode-hook= is used to set =“:comments”= header argument
to =“link”=, so that =org= exports comments along with source code when the
=vhdl-tools-tangle-comments-link= flag is set
...
Note that the variables =vhdl-tools-vorg-tangle-comment-format-beg= and
